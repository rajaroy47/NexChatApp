...
const UserAvatar = ({ name, size = 'md', online }) => {
  ...
  return (
    <div className="relative flex-shrink-0">
      ...
      {online !== undefined && (
        <span className={`absolute -bottom-[1px] -right-[1px] ${dotSz} rounded-full border-[2px] border-[#0c0c0c] ${online ? 'bg-emerald-500' : 'bg-gray-600'}`} />
      )}
    </div>
  );
};
        ...
        <div className="flex-shrink-0 px-4 pt-5 pb-3 border-b border-white/[0.04]">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search"
              style={{ fontSize: '16px' }}
              className="w-full pl-9 pr-4 py-2 rounded-xl bg-[#1a1a1a] text-[14px] text-white placeholder-gray-600 border border-white/[0.06] focus:outline-none focus:border-violet-500/40 transition-all"
            />
            {search && (
              <button
                onClick={() => setSearch('')}
        ...
        <div className="flex-1 overflow-y-auto nexchat-scrollbar py-1">
          ...
          {sorted.length > 0 && (
            <div className="px-5 py-2 mt-1">
              <span className="text-[11px] font-semibold text-gray-600 uppercase tracking-[0.8px]">Direct Messages</span>
            </div>
          )}
          ...
          <div className="px-2 space-y-0.5">
            {sorted.map(user => {
              ...
              return (
                <button
                  key={user.id}
                  onClick={() => pick(user)}
                  className={`w-full flex items-center gap-3 px-3 py-[11px] rounded-2xl transition-all duration-150 text-left ${
                    selected
                      ? 'bg-violet-600/15 ring-1 ring-violet-500/25'
                      : 'hover:bg-white/[0.04] active:bg-white/[0.07]'
                  }`}
            ...
            {sorted.length === 0 && search && (
              <div className="py-12 text-center">
                ...
                <p className="text-[13px] text-gray-500">No results for &ldquo;{search}&rdquo;</p>
              </div>
            )}
