  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      `}>
        ...
        <div className="px-5 sm:px-6 pt-5 sm:pt-5 pb-4 sm:pb-4 border-b border-white/[0.05]">
          <div className="flex items-center justify-between">
            ...
            <button 
              onClick={onClose} 
              className="w-9 h-9 rounded-xl bg-white/[0.05] hover:bg-white/[0.08] text-[#9999b0] hover:text-white transition-colors flex items-center justify-center active:scale-95 flex-shrink-0 ml-2"
              aria-label="Close"
            >
          ...
          {/* Tabs - Centered */}
          {!isInitialSetup && (
            <div className="flex gap-1.5 mt-4 bg-white/[0.03] rounded-2xl p-1 max-w-md mx-auto sm:mx-0">
              {['profile','security'].map(tab => (
                <button
                  key={tab}
                  onClick={() => setActiveTab(tab)}
                  className={`
                    flex-1 py-2.5 px-3 rounded-xl text-xs font-semibold capitalize transition-all
                    ${activeTab === tab 
        ...
        <div 
          className="px-5 sm:px-6 py-5 sm:py-5 overflow-y-auto nexchat-scrollbar" 
          style={{ maxHeight: isMobile ? 'calc(90vh - 140px)' : 'calc(85vh - 140px)' }}
        >
          {/* User card - Better centered layout */}
          <div className="flex flex-col items-center sm:flex-row sm:items-center gap-4 p-4 bg-white/[0.03] rounded-2xl border border-white/[0.05] mb-5">
            <div 
              className="w-16 h-16 sm:w-12 sm:h-12 rounded-full flex items-center justify-center text-xl sm:text-lg font-bold text-white flex-shrink-0" 
          ...
          {/* Profile tab - Centered form */}
          {(activeTab === 'profile' || isInitialSetup) && (
            <div className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-[#9999b0] uppercase tracking-wide block mb-2 text-center sm:text-left">
                  Display Name
                </label>
                <input
                  type="text"
                  ...
                  style={{ fontSize: '16px' }}
                  className="
                    w-full px-4 py-3.5 rounded-2xl text-center sm:text-left
                    bg-white/[0.04] border border-white/[0.08] text-white 
                    placeholder-[#55556a] focus:outline-none focus:border-violet-500/50 
                    focus:bg-white/[0.06] transition-all text-sm
                  "
                />
                <p className="text-xs text-[#55556a] mt-2 text-center sm:text-left">
                  Current: <span className="text-violet-400 font-medium">{userInfo.displayName}</span>
                  {isDefaultName(userInfo.displayName) && (
                    <span className="ml-2 text-amber-500">(Default)</span>
                  )}
                </p>
              </div>
              
              {messages.profile && (
                <div className={`px-4 py-3 rounded-xl border text-sm text-center ${msgCls(messages.profile)}`}>
                  {messages.profile}
                </div>
              )}
              ...
              <button
                ...
                className="
                  w-full py-3.5 btn-gradient text-white font-semibold 
          ...
          {activeTab === 'security' && !isInitialSetup && (
            <div className="space-y-4">
                ...
                <div key={key}>
                  ...
                  <div className="relative">
                    <input
                      ...
                      style={{ fontSize: '16px' }}
                      className="
                        w-full px-4 py-3.5 pr-12 rounded-2xl text-center sm:text-left
                        bg-white/[0.04] border border-white/[0.08] text-white 
                    ...
                    <button 
                      ...
                      className="absolute right-4 top-1/2 -translate-y-1/2 text-[#55556a] hover:text-[#9999b0] p-1"
              ...
              {messages.password && (
                <div className={`px-4 py-3 rounded-xl border text-sm text-center ${msgCls(messages.password)}`}>
                  {messages.password}
                </div>
              )}
              ...
              <button
                ...
                className="
                  w-full py-3.5 bg-gradient-to-r from-red-500 to-pink-600 
          ...
          {!isInitialSetup && (
            <div className="mt-6 pt-5 border-t border-white/[0.05]">
              <p className="text-xs font-semibold text-[#55556a] uppercase tracking-widest mb-4 text-center">
              ...
              <div className="flex justify-center gap-3">
                {SOCIALS.map(s => (
                  <a 
                    ...
                    className="
                      group relative flex flex-col items-center gap-1.5 p-3 
                      bg-white/[0.03] hover:bg-white/[0.06] rounded-xl 
                      border border-white/[0.05] hover:border-white/[0.1] 
                      transition-all overflow-hidden active:scale-95 w-16
                    "
                  >
                    ...
                    <span className="relative text-[10px] text-[#55556a] group-hover:text-[#9999b0] transition-colors">
                      {s.name}
                    </span>
                  </a>
                ))}
